...
export default function EstatePlanningChecklist() {
  ...
  return (
    <div className="bg-white border border-[#bee3f8] shadow-lg overflow-hidden">
      {/* Header */}
      <div className="bg-[#2c5282] p-6">
        <h3
        ...
        <div className="mt-4">
          <div className="flex justify-between text-xs text-white/60 mb-2" style={{ fontFamily: "'Lato', sans-serif" }}>
            ...
            <span className="text-[#2c5282] font-bold">{pct}%</span>
          </div>
          <div className="h-2 bg-white/10 rounded-full overflow-hidden">
            <div
              className="h-full bg-[#2c5282] transition-all duration-500 rounded-full"
              style={{ width: `${pct}%` }}
            />
          </div>
        </div>
      ...
      <div className="divide-y divide-[#f0ede8]">
        {checklistItems.map((item) => {
          ...
          return (
            ...
            >
              {isChecked ? (
                <CheckCircle size={20} className="text-[#2c5282] flex-shrink-0 mt-0.5" />
              ) : (
                <Circle size={20} className="text-[#ccc] flex-shrink-0 mt-0.5" />
              )}
              <div>
                <p
                  className={`text-sm font-bold ${isChecked ? "text-[#888] line-through" : "text-[#2c5282]"}`}
      ...
      {/* Result / CTA */}
      <div className="p-6 bg-[#ebf4ff] border-t border-[#bee3f8]">
        {pct === 100 ? (
          <div className="text-center">
            <p className="text-[#2c5282] font-bold mb-2" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.2rem" }}>
              Excellent — your plan looks complete!
            </p>
            <p className="text-[#4a6fa5] text-sm mb-4" style={{ fontFamily: "'Lato', sans-serif" }}>
          ...
          <div>
            <p className="text-[#2c5282] font-bold mb-2 text-sm" style={{ fontFamily: "'Lato', sans-serif" }}>
              {missing.length} item{missing.length > 1 ? "s" : ""} to address:
            </p>
            <ul className="space-y-1 mb-4">
              {missing.slice(0, 3).map((item) => (
                <li key={item.id} className="flex items-center gap-2 text-xs text-[#4a6fa5]" style={{ fontFamily: "'Lato', sans-serif" }}>
                  <ChevronRight size={12} className="text-[#2c5282]" />
                  {item.label}
                </li>
              ))}
              {missing.length > 3 && (
